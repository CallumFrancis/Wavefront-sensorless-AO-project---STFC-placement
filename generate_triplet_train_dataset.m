function generate_triplet_train_dataset(dataset_size,resolution,orders)
    ...
    mkdir(fullfile('Recognition_Dataset_nonroot','Train'))
    mkdir(fullfile('Recognition_Dataset_nonroot','Train','Labels'))
    mkdir(fullfile('Recognition_Dataset_nonroot','Train','Image_A'))
    mkdir(fullfile('Recognition_Dataset_nonroot','Train','Image_P'))
    mkdir(fullfile('Recognition_Dataset_nonroot','Train','Image_N'))
    ...
    parfor worker = 1:workers
        ...
        for i = start_idx:final_idx
            ...
            fdir = fullfile('Recognition_Dataset_nonroot','Train','Image_A',fname);
            imwrite(farfield_A,fdir);
            
            fname = sprintf('Image_P_%i.png',i);
            fdir = fullfile('Recognition_Dataset_nonroot','Train','Image_P',fname);
            imwrite(farfield_P,fdir);
            
            fname = sprintf('Image_N_%i.png',i);
            fdir = fullfile('Recognition_Dataset_nonroot','Train','Image_N',fname);
            imwrite(farfield_N,fdir);
            
%             fname = sprintf('Label_%i.csv',i);
%             fdir = fullfile('Recognition_Dataset','Train','Labels',fname);
