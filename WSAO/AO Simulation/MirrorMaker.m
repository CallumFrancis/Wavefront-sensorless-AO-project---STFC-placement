classdef MirrorMaker < handle
    ...
    properties
        ...
        default_influence = 0.09	% Default inlfuence value
    ...
    methods
        ...
        function mouse_move(self,~,~)
            C = get (self.AXES, 'CurrentPoint');
            title(self.AXES, ['(x,y) = (', num2str(C(1,1)), ', ',num2str(C(1,2)), ')']);
        end
...
%             self.influence_vector(self.number_channels) = self.scroll_value + self.default_influence;
        ...
        function mouse_click(self,varargin)
            %% Mouse click callback

            x = varargin{2};
            if isa(x,'double')
                y = varargin{3};
                self.last_click = [x;y]; % case for entering specific coordinates
            else
                self.last_click = get(self.AXES,'CurrentPoint'); % normal clicks
            ...
            self.influence_function = @(x,y) 1 * exp((-x.^2-y.^2)/...
                self.influence_vector(self.number_channels));
            influence = generate_influences2(self.resolution,...
        ...
        function key_pressed(self,~,event)
            ...
            switch event.Key
                ...
                case 'l'    % Load config
                    ...
                    self.update_influence('all')
                    self.update_figure();
                case 'c'    % Enter actuator coordinates
                    self.coord_entry()
        ...
        function save_mirror(self)
            ...
            save(fullfile(folder.path,fname),'distribution','influences','influence_values');
        end
        ...
        function coord_entry(self)
            prompt = {'Enter x coordinate:','Enter y coordinate'};
            dlgtitle = 'Coordinates';
            dims = [1 35];
            definput = {'0','0'};

            coordinates = inputdlg(prompt,dlgtitle,dims,definput);

            coordinates = str2double(coordinates);

            self.mouse_click(self,coordinates(1),coordinates(2));
        end
