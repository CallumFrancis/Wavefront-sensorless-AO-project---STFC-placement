function generate_validation_dataset(dataset_size,resolution,orders)
...
%         line4 = sprintf("Do you wish  to continue ?");
  ...
  tic
    ...
    mkdir(fullfile('Dataset','Test','Images'))
    mkdir(fullfile('Dataset','Test','Labels'))
    ...
    parfor worker = 1:workers
        ...
        for i = start_idx:final_idx
            ...
            fdir = fullfile('Dataset','Test','Labels',fname);   % Full path to save it to ".\Dataset\Responses\coeffs_index.csv".
            writematrix(coeffs(1:12),fdir);                       % Write file
            
            fname = sprintf("farfield_%i.png",i);               % Name of farfield image, "farfield_idex.png"
            fdir = fullfile('Dataset','Test','Images',fname);    % Full path to save it to ".\Dataset\Observations\farfield_idex.png".
            imwrite(ff,fdir);                                   % Write file
        end
    end
    end_time = toc;
    fprintf("Testing set generation duration: %.2f sec\n",end_time);
end