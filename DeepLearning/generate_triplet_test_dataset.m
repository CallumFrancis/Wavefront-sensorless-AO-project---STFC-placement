function generate_triplet_test_dataset(dataset_size,resolution,orders)
    ...
    mkdir(fullfile('Triplet_Dataset_nonroot','Test'))
    mkdir(fullfile('Triplet_Dataset_nonroot','Test','Labels'))
    mkdir(fullfile('Triplet_Dataset_nonroot','Test','Image_A'))
    mkdir(fullfile('Triplet_Dataset_nonroot','Test','Image_P'))
    mkdir(fullfile('Triplet_Dataset_nonroot','Test','Image_N'))
    ...
    parfor worker = 1:workers
        ...
        for i = start_idx:final_idx
            ...
            fdir = fullfile('Triplet_Dataset_nonroot','Test','Image_A',fname);
            imwrite(farfield_A,fdir);
            
            fname = sprintf('Image_P_%i.png',i);
            fdir = fullfile('Triplet_Dataset_nonroot','Test','Image_P',fname);
            imwrite(farfield_P,fdir);
            
            fname = sprintf('Image_N_%i.png',i);
            fdir = fullfile('Triplet_Dataset_nonroot','Test','Image_N',fname);
            imwrite(farfield_N,fdir);
            
%             fname = sprintf('Label_%i.csv',i);
%             fdir = fullfile('Triplet_Dataset','Test','Labels',fname);
